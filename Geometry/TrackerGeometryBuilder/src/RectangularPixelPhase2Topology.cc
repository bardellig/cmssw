...
float RectangularPixelPhase2Topology::localX(const float mpx) const {
  ...
  float lpX = float(binoffx * m_pitchx) + fractionX * local_pitchx + ispix_secondhalf_x*2*m_BIG_PIX_PITCH_X*m_nrows/m_ROWS_PER_ROC + m_xoffset;
...
float RectangularPixelPhase2Topology::localY(const float mpy) const {
  ...
  float lpY = float(binoffy * m_pitchy) + fractionY * local_pitchy + ispix_secondhalf_y*m_BIG_PIX_PITCH_Y*m_ncols/m_COLS_PER_ROC +  m_yoffset;
